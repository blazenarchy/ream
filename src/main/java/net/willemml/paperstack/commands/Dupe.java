...
public class Dupe implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!(sender instanceof ConsoleCommandSender)) {
            ...
            if (playerData.getTokens() < PaperStack.dupeCost) {
                GeneralUtilities.notEnoughTokens(player, PaperStack.dupeCost);
                return true;
            }
            Inventory inventory;
            ...
            playerData.removeTokens(PaperStack.dupeCost).increaseLifeTimeDupes().save();
